...
function isPalindrome(str) {
  const strconverted = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return strconverted === strconverted.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  const newSentence = sentence.replace(punctuationRegex, '');
  const arr = newSentence.split(' ');
  let maxLength = 0;
  let result = '';
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (/[a-zA-Z]/.test(symbol)) {
      const base = symbol <= 'Z' ? 65 : 97;
      const code = ((str.charCodeAt(i) - base + 13) % 26) + base;
